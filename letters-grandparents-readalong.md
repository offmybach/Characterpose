# Grandparent & Read-Along Letters

Six letters. Five to named contacts, one reusable. Contacts and addresses are in
`contacts-grandparents-media.md`.

**The spine of all six.** The book is catalogued **Lexile AD 620L**. The AD stands for
**Adult Directed** — the book industry's own label for a title meant to be read *with* a
...
cruise-ship price match and the one-cent clearance sticker are two separate stories and
neither belongs in this lane — leave both out.
...
`ccrosby@grandmagazine.com`, cc `editor@grandmagazine.com`

**Subject:** `For September: the money lesson a grandkid uses on Saturday`
...
`lieber@nytimes.com` (the address he publishes for NYT pitches on `ronlieber.com/contact`)

**The biggest miss in the whole contact set.** He was in none of the 620. Pitch the
argument. He can write the column without ever mentioning the book, and that's fine.

**Subject:** `The kids' money category skipped the cash register`
...
But saving is the second thing a child does with money; the first is spending. A six-year-old has no savings rate and no income. What she has done is stand at a register holding a birthday five-dollar bill and decide. Spending is the first financial act a person performs, and the entire category skipped it for forty years.
...
## 6. The reusable one — "Read It WITH Them"

For the email list, an insert card in shipped books, a handout at readings, a newsletter
...
## Do-not-alter list, all six

- **$19.99 exact.** Never round it, never discount it in copy.
- **Page references are real:** 6 (the 529), 24 (Dad levels with him), 27 (grocery game),
  28 (Guess the Price). Don't shift them.
- **AD 620L is estimated** in formal spec contexts — write "Est. Lexile AD 620L" on any
  fact sheet or cataloging record. In these conversational letters it reads clean.
- **No Aisle Five reveal and no last-name wordplay,** even if someone asks for the best
  scene. Tease the orange stickers and the markdown; guard the moment.
- **Neither shoe story appears here.** The cruise-ship price match and the one-cent
  clearance sticker are separate events, and "negotiated" is a banned verb. Leave both out
  of this lane entirely.
- **Endorsements are deliberately absent from letter 6.** Parents and grandparents don't
  buy off credentials, they buy off the couch scene. Don't add them.
