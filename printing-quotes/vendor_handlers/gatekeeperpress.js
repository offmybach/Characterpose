...
async function getQuote(page, config, quantity, screenshotDir) {
  ...
  try {
    await page.goto('https://gatekeeperpress.com/printing-costs/', {
    ...
    const ssPath = path.join(screenshotDir, `GatekeeperPress_q${quantity}.png`);
    ...
    if (result.print_price) {
      ...
      result.notes = 'Could not extract price. Gatekeeper Press has a cost calculator — visit site for hardcover, 8.5x11, 36pp, full color.';
    }
    ...
    const ssPath = path.join(screenshotDir, `GatekeeperPress_error_q${quantity}.png`);
