...
async function getQuote(page, config, quantity, screenshotDir) {
  ...
  try {
    await page.goto('https://www.diggypod.com/book-printing-quote.html', {
      waitUntil: 'domcontentloaded',
      timeout: config.browser.timeout_ms,
    });
    await page.waitForTimeout(3000);

    // DiggyPOD has a multi-step quote form
    ...
    const ssPath = path.join(screenshotDir, `DiggyPOD_q${quantity}.png`);
    ...
    if (result.print_price) {
      ...
      result.notes = 'Could not extract price. DiggyPOD has a quote form — visit site for hardcover, 8.5x11, 36pp, full color, gloss.';
    }
    ...
    const ssPath = path.join(screenshotDir, `DiggyPOD_error_q${quantity}.png`);
