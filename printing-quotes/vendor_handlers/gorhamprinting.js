...
async function getQuote(page, config, quantity, screenshotDir) {
  ...
  try {
    // Gorham has a direct quote calculator
    await page.goto('https://gorhamprinting.com/quoting/quote?go=new', {
    ...
    for (const sel of allSelects) {
      ...
      if (/bind|cover\s*type|book\s*type/i.test(label)) {
    ...
    // Re-fetch after binding selection may have changed the form
    ...
    for (const sel of selects2) {
      ...
      if (/color|ink|interior/i.test(label)) {
        for (const opt of options) {
          const val = await opt.textContent().catch(() => '');
          if (/full\s*color|4.*color|color\s*throughout/i.test(val)) {
            await sel.selectOption({ label: val.trim() });
            break;
          }
        }
    ...
    for (const btn of buttons) {
      ...
      if (/calculate|get.*quote|submit|price/i.test(text)) {
        await btn.click();
        break;
      }
    }
    ...
    const ssPath = path.join(screenshotDir, `GorhamPrinting_q${quantity}.png`);
    await page.screenshot({ path: ssPath, fullPage: true });
    result.screenshot_path = ssPath;

    const bodyText = await page.textContent('body').catch(() => '');

    const priceMatch = bodyText.match(/(?:total|price|cost|quote)[:\s]*\$?([\d,]+\.?\d{0,2})/i)
    ...
    if (result.print_price) {
      ...
      result.notes = 'Could not extract price. Gorham Printing has an online calculator — visit site for hardcover, 8.5x11, 36pp, full color, gloss.';
    }
    ...
    const ssPath = path.join(screenshotDir, `GorhamPrinting_error_q${quantity}.png`);
