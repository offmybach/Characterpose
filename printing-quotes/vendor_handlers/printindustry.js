...
async function getQuote(page, config, quantity, screenshotDir) {
  ...
  try {
    // PrintIndustry has a book printers page with an online bid/quote system
    await page.goto('https://www.printindustry.com/buy-printing/book-printers.aspx', {
      waitUntil: 'domcontentloaded',
      timeout: config.browser.timeout_ms,
    });
    await page.waitForTimeout(3000);

    // Look for an instant quote or configure link
    const links = await page.$$('a');
    for (const link of links) {
      const text = await link.textContent().catch(() => '');
      if (/instant\s*quote|configure|calculate|order|on[\s-]*demand|color\s*book/i.test(text)) {
    ...
    // Interact with any form elements
    ...
    for (const sel of allSelects) {
      ...
      if (/color|ink/i.test(label)) {
        for (const opt of options) {
          const val = await opt.textContent().catch(() => '');
          if (/full\s*color|color|4.*4/i.test(val)) {
            await sel.selectOption({ label: val.trim() });
            break;
          }
        }
    ...
    for (const btn of buttons) {
      ...
      if (/calculate|get.*quote|get.*price|submit|next/i.test(text + ' ' + val)) {
        await btn.click();
        break;
      }
    }
    ...
    const ssPath = path.join(screenshotDir, `PrintIndustry_q${quantity}.png`);
    ...
    const perUnitMatch = bodyText.match(/(?:per\s*(?:book|copy|unit)|each)[:\s]*\$?([\d.]+)/i);
    if (perUnitMatch) {
    ...
    if (result.print_price) {
      ...
      result.notes = 'PrintIndustry uses a bid system — post your specs to get quotes from multiple printers. Specs: hardcover, 11x8.5, 36pp, full color, 100# gloss text.';
    }
    ...
    const ssPath = path.join(screenshotDir, `PrintIndustry_error_q${quantity}.png`);
