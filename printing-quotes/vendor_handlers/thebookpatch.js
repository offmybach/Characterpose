...
async function getQuote(page, config, quantity, screenshotDir) {
  ...
  try {
    await page.goto('https://www.thebookpatch.com/printcalculator/', {
    ...
    for (const sel of allSelects) {
      ...
      if (/color|ink|interior/i.test(label)) {
        for (const opt of options) {
          const val = await opt.textContent().catch(() => '');
          if (/full\s*color|color/i.test(val)) {
            await sel.selectOption({ label: val.trim() });
            break;
          }
        }
    ...
    // Calculate
    const buttons = await page.$$('button, input[type="submit"], input[type="button"], a.btn');
    for (const btn of buttons) {
      const text = await btn.textContent().catch(() => '');
      const val = await btn.getAttribute('value').catch(() => '') || '';
      if (/calculate|get.*price|submit|quote/i.test(text + ' ' + val)) {
        await btn.click();
        break;
      }
    }
    ...
    const ssPath = path.join(screenshotDir, `TheBookPatch_q${quantity}.png`);
    await page.screenshot({ path: ssPath, fullPage: true });
    result.screenshot_path = ssPath;

    const bodyText = await page.textContent('body').catch(() => '');

    const priceMatch = bodyText.match(/(?:total|price|cost|per\s*book)[:\s]*\$?([\d,]+\.?\d{0,2})/i)
    ...
    if (result.print_price) {
      ...
      result.notes = 'Could not extract price. The Book Patch has a print calculator — visit site for hardcover, 8.5x11, 36pp, full color.';
    }
    ...
    const ssPath = path.join(screenshotDir, `TheBookPatch_error_q${quantity}.png`);
